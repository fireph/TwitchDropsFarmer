  ...
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm mb-8">
    ...
    <div class="p-6">
      <div class="space-y-6">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            Games to Exclude from Drop Farming
          </label>
          <div class="flex space-x-2 mb-2">
            <input
              v-model="newExcludeGameName"
              type="text"
              placeholder="Enter game name to exclude (e.g., Just Chatting)"
              class="flex-1 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg px-3 py-2 focus:ring-2 focus:ring-red-500 focus:border-transparent"
              @keydown.enter="addExcludeGame"
            >
            <button
              @click="addExcludeGame"
              :disabled="!newExcludeGameName.trim() || minerStore.isLoading"
          ...
          <div class="space-y-2">
            <draggable 
              v-model="localConfig.exclude_games" 
              group="exclude_games"
              @end="onExcludeGameReorder"
              item-key="id"
              class="space-y-2"
            >
              <template #item="{ element: game }">
                <div class="flex items-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800 hover:border-red-300 dark:hover:border-red-700 transition-colors">
                  <svg class="w-5 h-5 text-gray-400 cursor-grab mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M7 2a2 2 0 1 0 0 4 2 2 0 0 0 0-4zM7 8a2 2 0 1 0 0 4 2 2 0 0 0 0-4zM7 14a2 2 0 1 0 0 4 2 2 0 0 0 0-4zM13 2a2 2 0 1 0 0 4 2 2 0 0 0 0-4zM13 8a2 2 0 1 0 0 4 2 2 0 0 0 0-4zM13 14a2 2 0 1 0 0 4 2 2 0 0 0 0-4z"></path>
                  </svg>
                  <div class="flex-1">
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">
            Games listed here will be excluded from drop farming, even if they have available campaigns. Drag items to reorder.
          </p>
        </div>
...
async function addExcludeGame() {
  if (!newExcludeGameName.value.trim()) return
  
  try {
    await minerStore.addGame(newExcludeGameName.value.trim(), false) // false means exclude game
    newExcludeGameName.value = ''
  } catch (error) {
    console.error('Failed to add exclude game:', error)
  }
}

async function removeExcludeGame(game: GameConfig) {
  try {
    // Remove from local config
    const index = localConfig.exclude_games.findIndex(g => g.id === game.id)
    if (index !== -1) {
      localConfig.exclude_games.splice(index, 1)
    }
    
    // Update the store config
    minerStore.config.exclude_games = localConfig.exclude_games
    await minerStore.saveConfig()
  } catch (error) {
    console.error('Failed to remove exclude game:', error)
  }
}
...
async function onExcludeGameReorder() {
  try {
    // Update the store config with the new order
    minerStore.config.exclude_games = localConfig.exclude_games
    await minerStore.saveConfig()
  } catch (error) {
    console.error('Failed to save exclude game order:', error)
  }
}
